...
Service.prototype.resolve = function(protocol, callback) {
	...
	this.avahi.server.resolveService(this.interface, this.protocol, this.name, this.type, this.domain, protocol || Avahi.PROTO_UNSPEC, 0, function(err, interface, protocol, name, type, domain, derp, derp2, address, port, txt, flags) {
		if (err)
			return callback(err);
		...
		var proceed = (function (browser) {
			...
			if (typeof options.type === "undefined") {
				this.server.serviceTypeBrowserNew(interface, protocol, domain, flags, function(err, internalBrowser) {
					...
					internalBrowser.on("itemNew", function(interface, protocol, name, type, flags) {
						browser.emit("discovery", new Service(self, { interface: interface, protocol: protocol, name: name, type: type, flags: flags }));
					}).on("itemRemove", function(interface, protocol, name, type, flags) {
				...
				this.server.serviceBrowserNew(interface, protocol, type, domain, flags, function(err, internalBrowser) {
					...
					internalBrowser.on("itemNew", function(interface, protocol, name, type, domain, flags) {
